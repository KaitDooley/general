...
def generate_round_keys(key_bits):
    ...
    C = key56[:28]
    D = key56[28:]
    print(f"C0 = {bits_to_str(C)}")
    print(f"D0 = {bits_to_str(D)}")
...
def f_function(R, K):
    ...
    for i in range(8):
        ...
        val = S_BOX[i][row][col]
        s.extend(int_to_bits(val, 4))
    # P permutation
    return permute(s, P)

def decrypt_des(ciphertext_bits, round_keys):
    perm = permute(ciphertext_bits, IP)
    L = perm[:32]
    R = perm[32:]
    ...
    for i in range(16):
        ...
        R = new_R
        print(f"\nRound {i+1} (K{16-i}):")
        print(f"  f output = {bits_to_str(f_out)}")
...
def main():
    ...
    for i, k in enumerate(round_keys):
        print(f"K{i+1:<6} {bits_to_str(k):<50}")
    
    # Step 2: DES Decryption
    print("\nStep 2: DES Decrytpion")
    
    plaintext_bits = decrypt_des(ciphertext_bits, round_keys)
    
    plaintext_bin = bits_to_str(plaintext_bits)
    print(f"\nPlaintext:  {plaintext_bin}")
    
    plaintext = ''
    for i in range(0, 64, 8):
        byte_val = bits_to_int(plaintext_bits[i:i+8])
